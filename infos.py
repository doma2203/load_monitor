...
po rdzeniach, procesach i obszarach monitorowania temperatury. I, oczywiscie, posiadaja ulubione i specjalnie dedykowane
'property'.'''
...
class RAMunit(MonitorPoint):
    ...
    def check(self):
        ...
        if self.mode is 'total':
            return self.findsize(10,5,'MemTotal',info[0])
        elif self.mode is 'free':
            # nie wiem, kto wymyslil taka ilosc spacji, wklepalam z autopsji :D
            return self.findsize(11,4,'MemFree',info[1])
        else:
            raise NotImplementedError

    def findsize(self,upperbound, lowerbound, parameter,text):
...
class Batteries:
    def __init__(self):
        ...
        self._batlist=[Battery(i)() for i in glob(self.path)]
